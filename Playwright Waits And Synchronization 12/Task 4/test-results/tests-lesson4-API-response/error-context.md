...
- Name: tests\lesson4.spec.js >> API response
- Location: tests\lesson4.spec.js:14:5
...
```ts
  1  | /* Write a test where:
  2  | You start waiting for /login
  3  | Click Login
  4  | Get the response
  5  | Print its status code
  6  | Assert that status is 200
  7  | Start with:
  8  | const responsePromise = page.waitForResponse(
  9  |     response => response.url().includes("/login")
  10 | );
  11 | */
  12 | 
  13 | import {test, expect} from "@playwright/test";
  14 | test("API response", async({page})=>{
  15 |     await page.goto("https://www.saucedemo.com/");
  16 | 
  17 |     await page.getByPlaceholder("Username").fill("standard_user");
  18 |     await page.getByPlaceholder("Password").fill("secret_sauce");
  19 | 
> 20 |     const responsePromise = page.waitForResponse(response=>response.url().includes("/login"));
     |                                  ^ Error: page.waitForResponse: Test timeout of 30000ms exceeded.
  21 | 
  ...
  25 | 
  26 |     console.log("Status code:", response.status());
  27 | 
